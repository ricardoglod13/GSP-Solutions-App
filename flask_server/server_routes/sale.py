...
def createSale():
    ...
    db_queries('insert', 'venta', 
        deudor = f"""{sale["deudor"]}""",
        acreedor = f"""{sale["acreedor"]}""", 
        items = f"""[]""", 
...
def addItemsS(code, cant, id_sale):
    new_data_sale = []
    aux = 0
...
def updateSale(id):
    ...
    if (sale["deudor"] != res["deudor"] 
        or sale["acreedor"] != res["acreedor"] 
        or sale["pago_inmediato"] != res["pago_inmediato"]):

        updateDeudaCV('venta', res["deudor"], res["acreedor"])
        updateDeudaCV('venta', sale["deudor"], sale["acreedor"])

    return f'Venta #{id} Actualizada'

@flask_routes.route('/sales/<code>/<cant>/<id_sale>', methods=['PUT'])
def updateItemS(code, cant, id_sale):
    #=============== Verificando si la venta ya tiene items agregados ===============#
    data_sale = db_queries('select', 'venta', where='id', where_value=id_sale, fields=['items'], fetch=0)
